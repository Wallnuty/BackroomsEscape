...
export class DayCareRoomManager {
    ...
    _setupRooms() {
        ...
        this.rooms.Playground = new PlaygroundRoom(this.scene, this.world, playgroundPos);
    ...
    _setupHallways() {
        ...
        // Playground → SignIn
        const start1 = playground.position.clone().add(new THREE.Vector3(playground.width/2, 0, 0));
        const end1 = signIn.position.clone().add(new THREE.Vector3(-signIn.width/2, 0, 0));
        this.hallways.push(new Hallway(this.scene, this.world, start1, end1, corridorHeight, this.corridorWidth));

        // SignIn → ExtraRoom
        const start2 = signIn.position.clone().add(new THREE.Vector3(signIn.width/2, 0, 0));
        const end2 = extra.position.clone().add(new THREE.Vector3(-extra.width/2, 0, 0));
        this.hallways.push(new Hallway(this.scene, this.world, start2, end2, corridorHeight, this.corridorWidth));
    }
